...
Called as a subprocess by poster_agent.py with CWD set to the agents/ directory.
Progress is emitted to stdout as JSON lines so the parent process can poll and
update the in-memory job dict without any shared state or IPC beyond stdout.
...
def main() -> None:  # noqa: C901
    ...
    try:
        from poster_tools.parse_raw import parse_raw, gen_image_and_table
        from poster_tools.gen_outline_layout import filter_image_table, gen_outline_layout_v2
        from poster_tools.tree_split_layout import (
            main_train, main_inference, get_arrangments_in_inches,
            split_textbox, to_inches
        )
        from poster_tools.gen_pptx_code import generate_poster_code
        from poster_tools.gen_poster_content import gen_bullet_point_content
