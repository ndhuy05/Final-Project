...
# Lock that serialises the run_code_with_utils → poster.pptx → read sequence
# inside get_snapshot_from_section so that parallel deoverflow threads do not
# corrupt the shared poster.pptx file.
...
def get_agent_config(model_type):
    ...
    if model_type == 'qwen':
        agent_config = {
            ...
            "model_config": QwenConfig(max_tokens=None).as_dict(),
            "model_platform": ModelPlatformType.DEEPINFRA,
        }
    elif model_type == 'gemini':
        agent_config = {
            "model_type": ModelType.DEEPINFRA_GEMINI_2_FLASH,
            "model_config": GeminiConfig(max_tokens=8192).as_dict(),
            "model_platform": ModelPlatformType.DEEPINFRA,
            'max_images': 99
        }
        ...
        agent_config = {
            ...
            "model_config": VLLMConfig(max_tokens=8192).as_dict(),
        ...
        agent_config = {
            ...
            "model_config": ChatGPTConfig(max_tokens=8192).as_dict(),
        ...
        agent_config = {
            ...
            'model_config': OpenRouterConfig(max_tokens=None).as_dict(),
        }
...
def is_website_filled(
    ...
    """Alias of is_poster_filled for use by the web pipeline."""
    return is_poster_filled(bounding_boxes, overall_width, overall_height, max_lr_margin, max_tb_margin)
...
async def rendered_dims(html: Path) -> tuple[int, int]:
    from playwright.async_api import async_playwright  # lazy import — only used by html_to_png
...
def html_to_png(html_abs_path, poster_width_default, poster_height_default, output_path):
    ...
    from playwright.sync_api import sync_playwright  # lazy import — only used by html_to_png
...
def get_snapshot_from_section(leaf_section, section_name, name_to_hierarchy, leaf_name, section_code, empty_poster_path='poster.pptx'):
    ...
    # Serialise: run_code writes poster.pptx; we must not let two threads race on it.
    with _snapshot_lock:
        run_code_with_utils(section_code, utils_functions)
        # The generated code writes poster.pptx; open that to add borders.
        poster = Presentation('poster.pptx')
    ...
    # Use leaf_name to look up the zoomed bbox; fall back to section_name if absent.
    zoom_key = leaf_name if leaf_name in locations else section_name
