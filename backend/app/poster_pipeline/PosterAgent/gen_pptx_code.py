...
def save_poster_code(output_file, utils_functions, presentation_object_name):
    ...
    output_file_safe = str(output_file).replace('\\', '/')
    code = utils_functions
    code = fr'''
# Save the presentation
save_presentation({presentation_object_name}, file_name="{output_file_safe}")
'''
    return code
...
'''
    ...
    if content is not None:
        ...
        # Use forward slashes so the path is a valid Python string literal on Windows
        tmp_name_safe = tmp_name.replace('\\', '/')
        ...
        if vertical_anchor:
            code += fr'''
fill_textframe({var_name}, json.load(open('{tmp_name_safe}', 'r')), vertical_anchor="{vertical_anchor}")
'''
        else:
            code += fr'''
fill_textframe({var_name}, json.load(open('{tmp_name_safe}', 'r')))
'''
