...
@tenacity
def ppt_to_images(file: str, output_dir: str, warning: bool = False, dpi=72, output_type='jpg'):
    """Convert the first slide of a PPTX to a JPEG named slide_0001.jpg using spire.presentation.

    spire.presentation is a pure-Python renderer with no external dependencies,
    replacing the previous LibreOffice-based approach which required a working
    soffice installation.

    The output filename is always slide_0001.jpg (JPEG) to match the naming
    convention that all pipeline callers (gen_outline_layout, wei_utils, etc.) expect.
    The output_type argument is retained for API compatibility but ignored.
    """
    import io as _io
    from spire.presentation import Presentation as _SpirePrs
    ...
    prs = _SpirePrs()
    try:
        prs.LoadFromFile(file)
        for i, slide in enumerate(prs.Slides):
            stream = slide.SaveAsImage()
            data = bytes(stream.ToArray())
            img = _PILImage.open(_io.BytesIO(data)).convert('RGB')
            img.save(pjoin(output_dir, "slide_0001.jpg"), 'JPEG')
            # Only the first slide is needed for layout/overflow detection
            return
    finally:
        prs.Dispose()
...
def wmf_to_images(blob: bytes, filepath: str):
    """Convert a WMF image blob to a JPEG file.

    Attempts conversion via Pillow first (works on Windows without LibreOffice).
    Falls back to a blank placeholder image so the pipeline continues even if
    the WMF cannot be decoded.
    """
    import io as _io
    from PIL import Image as _PILImage
    ...
    try:
        img = _PILImage.open(_io.BytesIO(blob)).convert("RGB")
        img.save(filepath, "JPEG")
        return
    except Exception:
        pass
    # Fallback: save a blank white placeholder so downstream code can continue
    import logging as _logging
    _logging.getLogger(__name__).warning(
        "wmf_to_images: could not decode WMF blob, writing blank placeholder for %s", filepath
    )
    _PILImage.new("RGB", (800, 600), color=(255, 255, 255)).save(filepath, "JPEG")
