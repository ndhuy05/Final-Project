...
location_info = add_border_and_labels(website, label_diameter_pt=80)
id_map = create_element_id_map(website)
import json
...
website.save("{}_bordered.pptx")
'''

save_helper_info_border = r'''
location_info = add_border(website)
import json
...
def create_website(width_inch=48, height_inch=36):
    """
...
def save_presentation(prs, file_name="website.pptx"):
    """
...
def get_visual_cues(name_to_hierarchy, identifier, website_path='website.pptx'):
    prs = pptx.Presentation(website_path)
    ...
    save_presentation(prs, file_name=f"tmp/website_<{identifier}>_hierarchy_1.pptx")

    prs = pptx.Presentation(website_path)

    add_border_hierarchy(prs, name_to_hierarchy, 1, border_width=10, fill_boxes=True)
    save_presentation(prs, file_name=f"tmp/website_<{identifier}>_hierarchy_1_filled.pptx")
    ...
    # Save the presentation to disk.
    save_presentation(prs, file_name=f"tmp/website_<{identifier}>_hierarchy_2.pptx")
    ...
    # Save the presentation to disk.
    save_presentation(prs, file_name=f"tmp/website_<{identifier}>_hierarchy_2_filled.pptx")

'''


documentation = r'''
create_website(width_inch=48, height_inch=36):
    """
    Create a new Presentation object, set its slide size (e.g., 48x36 inches).
...
save_presentation(prs, file_name="website.pptx"):
    """
    Save the current Presentation object to disk.
...
Example usage:
website = create_website(width_inch=48, height_inch=36)
...
image = add_image(slide, 'img', 10, 25, 30, 30, 'data/website_exp/pdf/attention/_page_3_Figure_0.jpeg')
...
save_presentation(website, file_name="website.pptx")
