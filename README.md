...
Question
  ...
  retrieve action (one per targeted paper for multi-doc queries):
    └─ OpenRouter Embeddings API → 4096-dim query vector
...
- **fastembed TextCrossEncoder** — reranking only (BAAI/bge-reranker-base, ONNX)
- **Qdrant Client** — local on-disk vector store (4096-dim)
- **OpenAI SDK** — OpenRouter-compatible client (chat, vision, embeddings)
...
│   │       ├── embedding_service.py      # OpenRouter embeddings API (async)
